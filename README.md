...
This is the **public demo** of the Voice Heist booth game — the same game and UI,
with the booth authentication layer (device gate, player sign-in, OAuth, admin
portal) removed so you can clone it, run it, and deploy your own in minutes. The
prize-based experience stays exclusive to the Deepgram booth.

## How it works

- The **browser** holds the low-latency audio WebSocket straight to Deepgram (via
  the `@deepgram/agents` SDK), plus a small control WebSocket to the brain.
- The **brain** (FastAPI) owns the game: the multi-agent handoff (Host → Briefer →
  one of four gatekeepers via `UpdatePrompt` / `UpdateSpeak`), function-call verdicts
  (`grant_request` / `deny_request`), fail-soft per-turn scoring, and a SQLite data
  model. It also mints a short-lived Deepgram token so the API key never reaches the
  browser.

Four heists: 🍕 The Order · 💸 The Refund · ☎️ The Receptionist · 🚪 The List.

## Run it

You need a free Deepgram key ($200 credit, no card: https://console.deepgram.com/signup)
— it must have at least **Member** permissions so it can mint grant tokens. An
Anthropic key is optional (it powers the per-turn scoring judge).
...
brain/
  ...
  agents.py     the four gatekeepers + Host/Briefer: prompts, voices, Settings
  session.py    per-connection game brain: routing, handoff, verdict
  judge.py      fail-soft per-turn scorer (WARM/WEAK; the win is the gatekeeper's grant)
...
## What's different from the booth build

Removed for the public demo: the device gate + player sign-in (`vh_gate` /
